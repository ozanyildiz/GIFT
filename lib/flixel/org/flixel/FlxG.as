package org.flixel
{
	...
	public class FlxG
	{
		...
		static public function rotateGame(rotation:int, reset:Boolean=false):void
		{
			if(reset)
			{
				_game.x = 0;
			...
			var m:Matrix=_game.transform.matrix;
			//var rotationPoint:Point = new Point(((width*_game._zoom)/2),((height*_game._zoom)/2));
			var rotationPoint:Point = new Point(((width*FlxCamera.defaultZoom)/2),((height*FlxCamera.defaultZoom)/2));
			m.translate( -rotationPoint.x, -rotationPoint.y);
			m.rotate(rotation*(Math.PI/180));
			m.translate( rotationPoint.x, rotationPoint.y);
			_game.transform.matrix=m;
		}
	}
}